...
export async function lireCompte() {
  ...
  // Identifiants réels UNIQUEMENT via variables d'environnement (Netlify). Les valeurs
  // ci-dessous ne sont que des repli de développement local — jamais de vrai secret ici.
  const email = (process.env.ADMIN_EMAIL || 'admin@local.test').trim().toLowerCase();
  const mdp = process.env.ADMIN_PASSWORD || 'changeme-dev-only';
